import type { SpectralCoin } from './coinTypes.ts';

const SPECTRAL_CARDS: ReadonlyArray<SpectralCoin> = [
  {
    type: 'spectral',
...
export function getRandomSpectralCoins(count: number): ReadonlyArray<SpectralCoin> {
  const shuffled = [...SPECTRAL_CARDS].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, Math.min(count, shuffled.length));
}

export function createSpectralCoin(enhancement: 'foil' | 'holographic' | 'polychrome', count: number): SpectralCoin {
  const spectralMap = {
    foil: {
