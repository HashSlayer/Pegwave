...
import { CoinViewer } from '../coins/CoinViewer.tsx';
import { useSound } from '../../sound/index.ts';
...
export function RoundView({
  ...
  const [showCoinViewer, setShowCoinViewer] = useState(false);
  ...
  const getPlayedCards = (): ReadonlyArray<Coin> => {
    ...
    <div className="flex h-screen bg-gray-900">
      ...
      <div className="absolute top-4 right-4 flex gap-2">
        <button
          ...
          onClick={() => setShowCoinViewer(true)}
        >
          View Deck
        </button>
        <button
      ...
      <CoinViewer
        deck={runState.deck}
        isOpen={showCoinViewer}
        onClose={() => setShowCoinViewer(false)}
      />
    </div>
  );
}