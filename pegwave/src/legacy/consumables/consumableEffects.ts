import type { RunState } from '../game/index.ts';
import { addCash, levelUpPokerHand, setHandSize, setMaxJokers, removeCash, addConsumable } from '../game/runState.ts';
import type { SpectralCoin, ArcanaCoin, PlayingCoin, CoinEnhancement } from '../coins/index.ts';
import { createCoin, RANKS, SUITS } from '../coins/index.ts';
import type { PlanetPlinkoCard, ConsumableCard } from './index.ts';
import { getRandomPlanetPlinkos } from './planetPlinko.ts';
import { getRandomJoker } from '../shop/index.ts';
import type { PokerHandKey } from '../scoring/index.ts';

interface ConsumableEffectContext {
  readonly selectedCards?: ReadonlyArray<PlayingCoin>;
...
export function applySpectralEffect(
  ...
  switch (card.effect.type) {
    ...
    case 'addHolographic':
    case 'addPolychrome': {
      const enhancement: CoinEnhancement = 
        ...
        : ((): RunState => {
            ...
            return !cardToDestroy 
              ...
              : ((): RunState => {
                  ...
                  const replacementCards: PlayingCoin[] = [];
                  const enhancements: ReadonlyArray<CoinEnhancement> = ['foil', 'holographic', 'polychrome', 'glass'];
                  
                  const createReplacementCard = (): PlayingCoin => {
                    ...
                    return createCoin(randomSuit, randomRank, randomEnhancement);
                  };
...
export function applyArcanaEffect(
  ...
  switch (card.effect.type) {
    ...
    case 'createPlanetPlinko': {
      const cards = getRandomPlanetPlinkos(2);
      return cards.reduce(
        (state, plinko) => addConsumable(state, plinko),
        runState,
      );
    }
...
export function applyPlanetPlinkoEffect(
  runState: RunState,
  _card: PlanetPlinkoCard,
): RunState {
  // Planet Plinkos apply ambience on the field when placed — wired in GameCanvas.
  return runState;
}
  ...
  switch (consumable.type) {
    ...
    case 'arcana':
      switch (consumable.effect.type) {
        ...
        case 'createPlanetPlinko':
          return runState.consumables.length < runState.maxConsumables;
      }
      break;
