...
import { getCoinChipValue } from '../coins/index.ts';
import type { EvaluatedHand, HandLevels, PokerHandKey } from './pokerHands.ts';
import type { BossBlind } from '../blinds/index.ts';
...
function calculateCardChips(
  scoringCards: ReadonlyArray<Coin>,
  ...
  return scoringCards.reduce(
    (sum, card) => {
      ...
      return sum + (shouldSkip ? 0 : getCoinChipValue(card));
    },
    0
  );
}
...
export function calculateBaseScore(evaluatedHand: EvaluatedHand, playedCards: ReadonlyArray<Coin>): ChipMult {
  const handChips = evaluatedHand.handType.baseChips;
  const cardChips = playedCards.reduce(
    (sum, card) => sum + getCoinChipValue(card),
    0
  );
...
export function determineGlassBrokenCoins(
...
export function getCoinEnhancementEffects(cards: ReadonlyArray<Coin>): ReadonlyArray<ScoringEffect> {
