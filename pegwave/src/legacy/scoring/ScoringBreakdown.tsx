...
import type { ScoringState } from '../game/roundState.ts';
import type { Joker } from '../shop/joker.ts';
import { evaluateAllJokers } from '../shop/joker.ts';
import { getCoinEnhancementEffects } from './scoring.ts';
...
export function ScoringBreakdown({ scoringState, jokers }: ScoringBreakdownProps): React.ReactElement {
  // Get all effects
  const enhancementEffects = getCoinEnhancementEffects(scoringState.playedCards);
  const jokerContext = {
  ...
  return (
    <div className="text-center animate-score-pop mb-8">
