import type { Coin, DrawPile } from '../coins';
import { drawCards, discardCards } from '../coins';
import type { EvaluatedHand, ChipMult, ScoringEffect } from '../scoring';
import { evaluatePokerHand, calculateBaseChipMult, calculateFinalScore, applyEffects, getCoinEnhancementEffects, determineGlassBrokenCoins } from '../scoring';
...
interface BaseRoundState {
  ...
  readonly hand: ReadonlyArray<Coin>;
...
export interface ScoringState extends BaseRoundState {
  ...
  readonly brokenGlassCards?: ReadonlyArray<Coin>;
}
...
function createPlayingState(
  state: SelectingHandState,
  playedCards: ReadonlyArray<Coin>
): PlayingState {
...
export function calculateScore(
  ...
  const enhancementEffects = getCoinEnhancementEffects(playedCards);
...
export function finishScoring(state: ScoringState): PlayedState | RoundFinishedState {
  ...
  const brokenGlassCards = determineGlassBrokenCoins(state.playedCards);
