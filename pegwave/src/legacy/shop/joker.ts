...
export interface JokerContext {
  readonly playedCards: ReadonlyArray<Coin>;
  readonly evaluatedHand: EvaluatedHand;
  readonly handsPlayed: number;
  readonly discardsRemaining?: number;
  readonly heldCards?: ReadonlyArray<Coin>;
}
...
export function evaluateJokerEffect(
  ...
  switch (joker.type) {
    ...
    case 'scalingMult': {
      ...
      // For additive scaling (like Flash Coin, Red Coin), use addMult
...
/**
 * The live registry of every joker the game can offer. It is seeded with the
 * built-in {@link JOKERS}, but any module can `jokerRegistry.register(...)`
 * more at load time — new jokers automatically flow into the shop and random
 * draws without touching this file. This is the "dynamic foundation" for
 * adding content over time.
 */
export const jokerRegistry: Registry<Joker> = new Registry<Joker>('joker').registerAll(JOKERS);

const FALLBACK_JOKER: Joker = {
  id: 'joker-default',
  name: 'Default Joker',
  description: '+4 Mult',
  rarity: 'common',
...
/**
 * Authoring helper for declaring a new joker. Pairs the joker data with the
 * `Joker` type so authors get full type-checking, and gives a single seam to
 * register custom content:
 *
 *   jokerRegistry.register(defineJoker({ id: 'house-1', ... }));
 */
export function defineJoker(joker: Joker): Joker {
  return joker;
}

function withFreshId(template: Joker): Joker {
  return {
    ...template,
...
export function getRandomJoker(rarity?: JokerRarity): Joker {
  const template =
    rarity === undefined
      ? jokerRegistry.pickRandom()
      : jokerRegistry.pickRandom(joker => joker.rarity === rarity) ?? jokerRegistry.pickRandom();

  return withFreshId(template ?? FALLBACK_JOKER);
}
