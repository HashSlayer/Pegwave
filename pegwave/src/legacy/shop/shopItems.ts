import type { Coin, CoinEnhancement, Suit, Rank } from '../coins';
...
const SHOP_PACKS: ReadonlyArray<PackItem> = [
  ...
  },
  {
    ...
    type: 'pack',
    name: 'Celestial Pack',
    description: 'Choose 1 of 3 Planet Plinkos',
    price: 6,
    packType: 'celestial',
    cardCount: 3,
  },
...
export function createCardPack(packType: 'standard' | 'arcana' | 'spectral' | 'celestial', count: number): ReadonlyArray<Coin> {
  ...
  return Array.from({ length: count }, () => {
    ...
    const enhancement = ((): CoinEnhancement | undefined => {
      const enhancements: CoinEnhancement[] = ['foil', 'holographic', 'polychrome'];
    ...
    return createCoin(finalSuit, finalRank, enhancement);
  });
}
